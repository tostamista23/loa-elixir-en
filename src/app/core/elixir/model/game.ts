...
function highlightHTML(input: string): string {
  ...
  return input.replace(regex, (match) => {
    ...
    // If the content between '<' and '>' is a negative numeric value, highlight in red
    if (/^-\d+$/.test(match)) {
      return `<span ${redColor}>${match}</span>`;
    }

    // Otherwise, highlight the text between '<' and '>' in yellow
    return `<span ${goldColor}>${match}</span>`;
  });
}
